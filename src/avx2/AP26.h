...
#define vec_sel(_X, _Y, _Z) \
	_mm256_or_si256( _mm256_andnot_si256( _Z, _X ), _mm256_and_si256( _Y, _Z ) )
...
{ 
	...
	__m256i svec		= _mm256_set_epi16(S59%101, S59%103, S59%107, S59%109, S59%113, S59%127, S59%131, S59%137, S59%61, S59%67, S59%71, S59%73, S59%79, S59%83, S59%89, S59%97);
	...
	//__m128i mvec2		= _mm_set_epi16(MOD%101, MOD%103, MOD%107, MOD%109, MOD%113, MOD%127, MOD%131, MOD%137);
        __m256i mvec		= _mm256_set_epi16(MOD%101, MOD%103, MOD%107, MOD%109, MOD%113, MOD%127, MOD%131, MOD%137,MOD%61, MOD%67, MOD%71, MOD%73, MOD%79, MOD%83, MOD%89, MOD%97);
	...
	//__m128i numvec2_2	= _mm_set_epi16(100, 102, 106, 108, 112, 126, 130, 136);
        __m256i numvec1		= _mm256_set_epi16(101, 103, 107, 109, 113, 127, 131, 137, 61, 67, 71, 73, 79, 83, 89, 97);
        __m256i numvec2		= _mm256_set_epi16(100, 102, 106, 108, 112, 126, 130, 136, 60, 66, 70, 72, 78, 82, 88, 96);
	...
	for(SHIFT=startSHIFT; SHIFT<(startSHIFT+640); SHIFT+=64){
		...
		//__m128i r_numvec2 = _mm_set_epi16(REM(n59,101,7), REM(n59,103,7), REM(n59,107,7), REM(n59,109,7), REM(n59,113,7), REM(n59,127,7), REM(n59,131,8), REM(n59,137,8));
                __m256i r_numvec = _mm256_set_epi16(REM(n59,101,7), REM(n59,103,7), REM(n59,107,7), REM(n59,109,7), REM(n59,113,7), REM(n59,127,7), REM(n59,131,8), REM(n59,137,8), REM(n59,61,6), REM(n59,67,7), REM(n59,71,7), REM(n59,73,7), REM(n59,79,7), REM(n59,83,7), REM(n59,89,7), REM(n59,97,7));
		...
		//				& OKOK137[_mm_extract_epi16(r_numvec2, 0)]))
                                if(sito = OKOK61[_mm256_extract_epi16(r_numvec, 7)]
                                                & OKOK67[_mm256_extract_epi16(r_numvec, 6)]
                                                & OKOK71[_mm256_extract_epi16(r_numvec, 5)]
                                                & OKOK73[_mm256_extract_epi16(r_numvec, 4)]
                                                & OKOK79[_mm256_extract_epi16(r_numvec, 3)]
                                                & OKOK83[_mm256_extract_epi16(r_numvec, 2)]
                                                & OKOK89[_mm256_extract_epi16(r_numvec, 1)]
                                                & OKOK97[_mm256_extract_epi16(r_numvec, 0)])
                                if(sito&=(OKOK101[_mm256_extract_epi16(r_numvec, 15)]
                                                & OKOK103[_mm256_extract_epi16(r_numvec, 14)]
                                                & OKOK107[_mm256_extract_epi16(r_numvec, 13)]
                                                & OKOK109[_mm256_extract_epi16(r_numvec, 12)]
                                                & OKOK113[_mm256_extract_epi16(r_numvec, 11)]
                                                & OKOK127[_mm256_extract_epi16(r_numvec, 10)]
                                                & OKOK131[_mm256_extract_epi16(r_numvec, 9)]
                                                & OKOK137[_mm256_extract_epi16(r_numvec, 8)]))
				...
				r_numvec = _mm256_add_epi16(r_numvec, svec);
				...
				if(n59>=MOD){
					...
					r_numvec = _mm256_sub_epi16(r_numvec, mvec);
					addvec = _mm256_add_epi16(r_numvec, numvec1);
					r_numvec = vec_sel(r_numvec, addvec, _mm256_cmpgt_epi16( zerovec, r_numvec) );
				}
				...
				subvec = _mm256_sub_epi16(r_numvec, numvec1);
				r_numvec = vec_sel(r_numvec, subvec, _mm256_cmpgt_epi16(r_numvec, numvec2) );
