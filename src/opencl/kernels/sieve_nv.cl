...
__kernel void sieve(__global long *n59, long S59, int shift, __global long *n_result, __global long *OKOK, __global int *ncount, int offset
	...
	localOKOK[get_local_id(0)] = OKOK[get_local_id(0)];			// copies 0-1023
	localOKOK[get_local_id(0) + 1024] = OKOK[get_local_id(0) + 1024];		// copies 1024 - 2047
	localOKOK[get_local_id(0) + 2048] = OKOK[get_local_id(0) + 2048];		// copies 2048 - 3071
	...
	if(i+offset < quartern59s){
		...
		for(i59=0;i59<35;i59++){
			...
			if(sito &= localOKOK[ ((n59a+77*n59b)%137) + 1411 ]
			...
			if(sito &= localOKOK[ ((n59a+25*n59b)%163) + 2144 ]
